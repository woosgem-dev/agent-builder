...
import { Badge, Card, CardBody, Button } from '@woosgem-dev/react';
...
const gradeConfig = {
  S: { label: '⭐ Premium', color: 'warning' as const },
  A: { label: '✅ Verified', color: 'success' as const },
  B: { label: '📋 Standard', color: 'primary' as const },
  C: { label: '⚠️ Basic', color: 'secondary' as const },
  D: { label: '❌ Incomplete', color: 'danger' as const },
};

function estimateCost(tokens?: number): string {
  if (!tokens) return '-';
  const cost = (tokens / 1_000_000) * 18;
...
export function SkillCard({
  ...
  return (
    <Card
      variant="outlined"
      hoverable
      className={isCompareSelected ? 'ring-2 ring-blue-500' : ''}
    >
      <CardBody>
        {/* Header */}
        ...
        <div className="flex items-center justify-between">
          <Link href={`/skills/${authorUsername}/${name}`}>
